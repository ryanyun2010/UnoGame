...
function PClass(classname){return document.querySelectorAll('.'+classname)}
function PQuery(type,query){if(type === 'all'){return document.querySelectorAll(query);}if(type === 'single'){return document.querySelector(query);}}
/********* Panda Js  End **********/
...
function useCard(parsedCardUsed, player) {
	...
	if (parseInt(PID("curplayerholder").innerHTML) === player) {
		...
		if (checkCard(parsedCurrentCard, parsedCardUsed)) {
			...
			if (parsedCardUsed[0].includes("Reverse")) {
				...
				PID("curplayerholder").innerHTML = player;
			...
			//checks for +2
				...
				if (player === 2) {
					randomUnoCard(CardNames, "add", PID("player1cards"), "height:200px", 2, 1)
				} else {
					randomUnoCard(CardNames, "add", PID("player2cards"), "height:200px", 2, 2)
				}
				PID("curplayerholder").innerHTML = player;
...
PID("popup").addEventListener("click",function(){
	if(PID("popup").style.display === "block"){
		popupbuttonclicked=true;
		PID("popup").style.display= "none";
	}
})
function flipCards(player,alerttrue){
	PID("player1cardsflipped").style.display = "block";
	PID("player2cardsflipped").style.display = "block";
	setTimeout(function(){if(alerttrue){
		
		PID("popup").style.display= "block";
	...
	var flipcardchecker = setInterval(function(){
		if(popupbuttonclicked){
			if(player===1){
				PID("player1cardsflipped").style.display = "block"
				PID("player2cardsflipped").style.display = "none"
			}
			else if(player===2){
				PID("player2cardsflipped").style.display = "block"
				PID("player1cardsflipped").style.display = "none"
			}
